...
export default async function handler(req, res) {
  ...
  const deterministicContext = {
    ...
    findings: safeArray(securityScan.findings).slice(0, 30).map(finding => ({
      ...
      fix: finding.fix,
      whyMatched: safeArray(finding.whyMatched).slice(0, 4),
      falsePositiveNotes: safeArray(finding.falsePositiveNotes).slice(0, 4),
    ...
    supplyChain: securityScan.sections?.supplyChain,
    sourceTools: safeArray(securityScan.sections?.sourceTools).slice(0, 8),
