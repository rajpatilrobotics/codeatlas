...
class ApiClient {
  ...
  async getBlastRadius(repositoryId, entityId) {
    return this.request(`/api/graph/blast-radius/${repositoryId}?entityId=${encodeURIComponent(entityId)}`)
  }

  async listGraphEntities(repositoryId, type) {
    const q = type ? `?type=${encodeURIComponent(type)}` : ''
    return this.request(`/api/graph/entities/${repositoryId}${q}`)
  }
