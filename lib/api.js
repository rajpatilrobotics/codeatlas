...
class ApiClient {
  ...
  async getHeatmap(repoId) {
    return this.request(`/api/graph/heatmap/${repoId}`)
  }

  async getDependencyTree(repoId, fileId) {
    return this.request(`/api/graph/dependency-tree/${repoId}?fileId=${fileId}`)
  }
  ...
  async createChatSession(repoId) {
